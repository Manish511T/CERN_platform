...
import DashboardSkeleton from "../components/DashboardSkeleton"; // ← import
...
const DashboardPage = () => {
  ...
  // Simulate a brief loading state for data fetch
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // In real usage this would wait for your API calls
    // Here we wait until user object is available
    if (user) {
  ...
  return (
    <PageWrapper
      loading={loading}
      skeleton={<DashboardSkeleton />} // ← pass skeleton
    >
      {/* ── Header ─────────────────────────────────────────────────────── */}
      ...
      >
        <Card
          padding="lg"
          className="bg-linear-to-br from-red-500 to-red-600
      ...
      >
        ...
        <Card padding="md" className="text-center">
          <div
            className="w-10 h-10 bg-green-50 rounded-xl flex items-center
                          justify-center mx-auto mb-2"
          >
        ...
        <Card padding="none" className="divide-y divide-slate-50">
          {[
            ...
            >
              ...
              <span className={`text-base font-bold ${color}`}>{number}</span>
            </a>
          ))}
        </Card>
      </motion.div>
